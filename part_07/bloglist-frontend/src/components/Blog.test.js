...
describe("<Blog />", () => {
  ...
  describe("Content appearance", () => {
    ...
    test("Likes and URL are hidden by default", async () => {
      ...
      const hiddenURLEl = screen.getByText(
        new RegExp(`^URL: ${blog.url}`, "i")
      );
      const hiddenLikesEl = screen.getByText(
        new RegExp(`^Likes: ${blog.likes}`, "i")
      );
  ...
  describe("Content interaction", () => {
    ...
    test("Likes and URL are shown when View button is clicked", async () => {
      ...
      const likesEl = screen.getByText(
        new RegExp(`^Likes: ${blog.likes}`, "i")
      );
